...
export default function StaffScheduling() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'schedule' | 'beds'>('schedule');
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1.5 rounded-full transition-all ${
                locked
                  ? 'text-amber-500 opacity-50 hover:opacity-100 hover:bg-amber-50'
                  : 'text-gray-400 opacity-20 hover:opacity-60 hover:bg-gray-100'
              }`}
              title={locked ? 'Unlock schedule (enable editing)' : 'Lock schedule (disable editing)'}>
        ...
        <div className="max-w-[1800px] mx-auto flex items-center gap-1">
          <button
            ...
            className={`px-4 py-2.5 text-xs font-bold rounded-t-lg transition-all border-b-2 ${
              activeTab === 'schedule'
                ? 'text-[#003153] border-[#003153] bg-[#003153]/5'
                : 'text-gray-400 border-transparent hover:text-gray-600'
            }`}>
            <span className="flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5" /> Shift Schedule</span>
          </button>
          <button
            ...
            <span className="flex items-center gap-1.5"><Bed className="w-3.5 h-3.5" /> Bed Allocation</span>
          </button>
        </div>
      </div>
                    ...
                    return (
                      <React.Fragment key={s.id}>
                        ...
                        <div className="sticky left-0 z-10 bg-white px-3 py-2 border-b border-gray-100 flex items-center gap-2.5 hover:bg-gray-50/50 transition-colors">
                          {s.profile_picture ? (
                            <img src={getMediaUrl(s.profile_picture)} alt="" className="w-8 h-8 rounded-xl object-cover ring-2 ring-white shadow-sm flex-shrink-0" />
                          ) : (
                            ...
                            <div key={di}
                              ...
                              {assignment ? (
                                <div className="group/cell relative">
                                  <button
                                    onClick={(e) => canEdit ? handleCellClick(s.id, key, e.currentTarget) : undefined}
                                    className={`px-2 py-1.5 rounded-lg text-[10px] font-bold text-white shadow-sm transition-shadow ${canEdit ? 'hover:shadow-md cursor-pointer' : 'cursor-default'}`}
                                    style={{ backgroundColor: assignment.shift_color }}
                                    title={`${assignment.shift_name}${canEdit ? ' (click to change/remove)' : ''}`}>
      ...
      {editingCell && dropdownPos && (() => {
        ...
        const editingAssignment = scheduleMap.get(staffId)?.get(date);
        return (
        <div
        ...
        >
          ...
          {editingAssignment && (
            <button
              onClick={() => { setEditingCell(null); setDropdownPos(null); handleDelete(editingAssignment.id); }}
            ...
            >
              <Trash2 className="w-3 h-3" /> Remove Shift
            </button>
          )}
          <button
